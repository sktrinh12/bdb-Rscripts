    ...
    if (trigger[[3]] == T && length(filter_param) > 4) {
      ...
      for (i in seq(1,nbr_wells)) {
        ...
        if (i %in% blanks) {
          ...
          if (i %% 12 == 0) {
            ...
            if (length(reset_lims) > 0) {
              ...
              for (k in cnt_reset_lims) {
                message(paste0('resetting: ', k))
                # have to retrieve the data again since not within same loop counter
                ii = reset_lims[j]
                gs <- Subset(fs[k], set_gate)
                gs <- GatingSet(gs)
                tf <- transformerList(param_name_list[ii], biexpTrans)
                tfd <- transform(gs, tf)
                da <- gs_cyto_data(tfd)
                g_pop2 <- openCyto:::.mindensity(da[[1]], channels = param_name_list[ii])
                p <- ggcyto(da, aes(x = !!param_name_list[ii]))
                if (!k %in% first_column) {
                  p <- p + 
                       geom_histogram(stat = "bin", bins = nbr_bins, fill = hst_fill_colour[ii]) +
                       geom_gate(g_pop2) + 
                       geom_stats(size = 1.5,
                  ...
                  p <- p + 
                    geom_histogram(stat = "bin", bins = first_colm_hist_cnt, fill = hst_fill_colour[ii])
                }
                

                plist2[[ii]] <- gcy_plot(p, max_x, max_y, top_idxs[j], left_idxs[j], wellIDs[k], param_name_list[ii])
                j <- j + 1
              }
            }
        ...
        tryCatch( {
          ...
          df_stats[cnt, "SAMPLE_NAME"] <- fcs_files_ordered[cnt]
          ...
          if (curr_ymax[2] > max_y) {
            message(paste0('requires resetting lims: ', cnt))
